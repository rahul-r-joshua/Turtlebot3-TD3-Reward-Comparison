...
#
# Authors: Ryan Shim, Gilbert, Tomas, Nandu, Rahul R
...
class DrlAgent(Node):
    def __init__(self, training, algorithm, load_session="", load_episode=0, real_robot=0):
        ...
        if self.algorithm == 'td3':
            ...
            quit("\033[1m" + "\033[93m" + f"invalid algorithm specified ({self.algorithm}), only 'td3' is allowed" + "\033[0m}")
        ...
        if self.load_session:
            ...
            if self.algorithm == 'td3':
                ...
                self.model.noise = OUNoise(
                    action_space=self.model.action_size,
                    max_sigma=0.1,
                    min_sigma=0.1,
                    decay_period=8000000
                )

                for param_group in self.model.actor_optimizer.param_groups:
                    param_group['lr'] *= 0.5
                for param_group in self.model.critic_optimizer.param_groups:
                    param_group['lr'] *= 0.5
                print(f"[INFO] TD3: Learning rates halved for fine-tuning continuation.")

            if self.training:
                try:
                    self.replay_buffer.buffer = self.sm.load_replay_buffer(
                        self.model.buffer_size,
                        os.path.join(self.load_session, f'stage{self.sm.stage}_latest_buffer.pkl')
                    )
                except EOFError:
                    print("[WARNING] Replay buffer file empty or corrupted. Starting with empty buffer.")
